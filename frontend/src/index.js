...
const onClickTaskDetail = (event) => {
  ...
  event.target.offsetParent.className = isActive
    ? "to-do-item"
    : "to-do-item to-do-item-active";
};
const onClickCheckBox = async (event) => {
  const labelClass = event.target.checked
    ? "fas fa-check-square"
    : "far fa-check-square";
...
const loadTaskByUserName = async (isAutoFocus = true) => {
  ...
  tasks.forEach((task) => {
    ...
    liEl.innerHTML = `<input class="none" onclick="onClickCheckBox(event)" type="checkbox" ${
      task.completed ? "checked" : ""
    } id="todo-chk-${task.id}" /><label for="todo-chk-${task.id}" class="${
      task.completed ? "fas" : "far"
    } fa-check-square"></label><span class="item-text" onclick="onClickTaskDetail(event)"> ${
      task.content
...
const onClickHiddenCompletedTask = () => {
  ...
  document.getElementById(
    "hidden-completed-task_icon"
  ).className = pageData.showCompletedTask
    ? "fas fa-check-circle"
    : "far fa-check-circle";
  loadTaskByUserName(false);
};
...
const onClickAllDelCompletedTask = async () => {
  ...
  for (let item of toDoItems) {
    if (item.firstChild.checked) await delTask(item.dataset.id);
  }
  loadTaskByUserName(false);
};
(async function init() {
  console.log("📣  Access success..!!");
  if (getLocalStoage("user"))
    User.instance = JSON.parse(getLocalStoage("user"));
